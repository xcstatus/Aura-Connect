...
use crate::iced_app::widgets::chrome_button::{style_chrome_primary, style_top_icon};

/// Build the vault unlock modal.
pub fn vault_unlock_modal(state: &IcedState) -> Element<'_, Message> {
    ...
    body = body.push(
        row![
            ...
            button(text(i18n.tr("iced.vault_unlock.btn.cancel")).size(13))
                .on_press(Message::VaultUnlockClose)
                .style(crate::iced_app::widgets::chrome_button::style_chrome_secondary(13.0)),
        ]
        .spacing(8),
    );
