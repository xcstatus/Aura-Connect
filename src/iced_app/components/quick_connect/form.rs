...
use crate::iced_app::chrome::top_bar_material_style;
use crate::iced_app::message::Message;
use crate::iced_app::state::IcedState;
use crate::iced_app::widgets::chrome_button::style_chrome_primary;
...
pub fn quick_connect_new_form(state: &IcedState) -> Element<'_, Message> {
    ...
    let is_connecting = matches!(flow, crate::iced_app::state::QuickConnectFlow::Connecting);
    ...
    let flow_banner: Option<Element<'_, Message>> = match flow {
        crate::iced_app::state::QuickConnectFlow::NeedUser => Some(
            container(text(
                err_kind
                    .unwrap_or(crate::app_state::ConnectErrorKind::MissingHostOrUser)
                    .user_message(),
            ))
            .padding(10)
        ...
        crate::iced_app::state::QuickConnectFlow::NeedAuthPassword => {
            let remaining = 3usize.saturating_sub(state.model.draft.password_error_count as usize);
            let msg = if err_kind == Some(crate::app_state::ConnectErrorKind::AuthFailed) {
                if remaining > 0 {
        ...
        crate::iced_app::state::QuickConnectFlow::AuthLocked => Some(
            container(text("SSH  密码多次错误，已中断本次连接。请编辑后重试或切换认证方式。"))
                .padding(10)
                .style(top_bar_material_style)
                .into(),
        ),
        crate::iced_app::state::QuickConnectFlow::Failed => Some(
            container(text(
                err_kind
                    .unwrap_or(crate::app_state::ConnectErrorKind::Unknown)
                    .user_message(),
            ))
            .padding(10)
            .style(top_bar_material_style)
    ...
    let connecting_progress: Option<Element<'_, Message>> = if is_connecting {
        ...
        let stage_label = match stage {
            crate::iced_app::state::ConnectionStage::VaultLoading => {
                state.model.i18n.tr("iced.stage.vault_loading")
            }
            crate::iced_app::state::ConnectionStage::SshConnecting => {
                state.model.i18n.tr("iced.stage.ssh_connecting")
            }
            crate::iced_app::state::ConnectionStage::Authenticating => {
                state.model.i18n.tr("iced.stage.authenticating")
            }
            crate::iced_app::state::ConnectionStage::SessionSetup => {
    ...
    let interactive_row: Option<Element<'_, Message>> = if matches!(
        ...
        crate::iced_app::state::QuickConnectFlow::NeedAuthInteractive
    ) {
    ...
    let actions = row![
        button(text(connect_btn_text).size(13))
            .on_press_maybe(
                (!matches!(
                    flow,
                    crate::iced_app::state::QuickConnectFlow::Connecting
                        | crate::iced_app::state::QuickConnectFlow::AuthLocked
                        | crate::iced_app::state::QuickConnectFlow::NeedAuthInteractive
                ))
    ...
    let header_row: Element<'_, Message> = if is_connecting {
        ...
        row![
            ...
            button(text("×").size(14))
                ...
                .style(crate::iced_app::widgets::chrome_button::style_top_icon(14.0)),
        ]
        .spacing(8)
        .align_y(Alignment::Center)
        .into()
    ...
    let switch_auth_btn: Option<Element<'_, Message>> = if matches!(
        flow,
        crate::iced_app::state::QuickConnectFlow::Failed
            | crate::iced_app::state::QuickConnectFlow::AuthLocked
    ) {
        Some(
