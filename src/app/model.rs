...
impl AppModel {
    ...
    pub fn new_for_connect(draft: ConnectionDraft, settings: Settings) -> Self {
        ...
        let tab_manager = SessionManager::new(store);
    ...
    pub fn load() -> Self {
        ...
        let mut tab_manager = SessionManager::new(store);

        if let Ok(content) = std::fs::read_to_string(&sessions_path) {
            if let Ok(library) = serde_json::from_str::<SessionLibrary>(&content) {
                tab_manager.library.sessions = library.sessions;
            }
        }
    ...
    pub fn profiles(&self) -> &[SessionProfile] {
        &self.tab_manager.library.sessions
    }
    ...
    pub async fn connect_from_draft(
        &mut self,
    ) -> std::result::Result<std::sync::Arc<SshChannel>, ConnectErrorKind> {
